...
public final class IndexSmoke {
    public static void main(String[] args)throws Exception{
        ...
        for(int iteration=0;iteration<2;iteration++)try(var storage=IndexStorage.open(root.resolve("index-v2"),8L*1024*1024)){
            var store=storage.store();long scan=storage.inventory().beginScan();
            ...
            if(iteration==1&&((Number)((Map<?,?>)storage.status().get("repository")).get("published")).longValue()!=0)throw new IllegalStateException("Reopen rewrote immutable artifact");
