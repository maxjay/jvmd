...
public final class IndexedFileManager extends ForwardingJavaFileManager<StandardJavaFileManager> {
    ...
    private void refreshSourceInventory()throws IOException {
    ...
    private Stamp stamp(Path path)throws IOException {
        return new Stamp(Files.getLastModifiedTime(path).to(java.util.concurrent.TimeUnit.NANOSECONDS),files.hash(path));
    }
