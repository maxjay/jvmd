...
public final class CodePass {
    ...
    public CodePass(IndexService index){
        this.index=index;
        ModuleLayer.boot().modules().forEach(module->module.getPackages().forEach(pkg->jdkPackages.put(pkg,module.getName())));
    }
    ...
    private synchronized long prepare(IndexStore.ArtifactCandidate candidate,boolean code)throws Exception{
        ...
        }else index.store().publishClassReferences(id,CodeReader.classReferences(content.models().values()));
        return id;
    }
    ...
    private void ensureJdk(String key)throws Exception{
        ...
        if(Files.isRegularFile(file)){
            ...
            if(artifact==null||!artifact.hasSignatureEdges())index.indexJdk(file,module,Path.of("/nonexistent-jvmd-src.zip"));
        }
    }
    ...
    public Expansion expand(List<Map<String,Object>> frontier,boolean outgoing,Set<String> kinds,String workspace)throws Exception{
        ...
        if(outgoing){
            ...
            for(var candidate:index.store().binaryArtifacts(workspace))prepare(candidate,false);
