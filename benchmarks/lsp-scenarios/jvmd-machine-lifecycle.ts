...
async function runDefinitionSession(daemon:Daemon,root:string,label:string,localChange:boolean){
  ...
  return {
    ...
    workspaceIndex:{
      ...
      indexServiceWorkDuringSessionOpen:deltaIndex(afterIndex,beforeIndex),
      counterScope:{
        reused:"dependency artifact reuse counter",
        hashed:"dependency artifact hash counter",
        indexedPublications:"aggregate IndexService publication counter; dependency and workspace-local publications are not separately exposed",
        parseMs:"IndexService parse worker time; no authoritative dependency/local attribution field is exposed",
        storageMs:"IndexService storage worker time; no authoritative dependency/local attribution field is exposed"
      },
...
async function main(){
  ...
  const residentDaemon={
    ...
    indexServiceDeltaDuringSessions:deltaIndex(settledIndex,residentIndexBefore),
    indexServiceCounterScope:"shared IndexService counters during resident sessions; scan/discovery are machine-repository lifecycle, while indexed publications are not separable into dependency versus workspace-local publications",
