...
public final class RocksIndexStore implements IndexStore {
    ...
    /**
     * General symbol search remains the user/navigation surface and must not expose private members
     * from installed/JDK artifacts. The semantic read path deliberately bypasses this filter through
     * exact-symbol and owner/member queries so access proofs can still reason over the full machine
     * declaration surface. Local workspace symbols remain searchable as before.
     */
    private static boolean searchVisible(StoredArtifact artifact,ArtifactIndexFormat.SymbolRecord symbol){
        return "local".equals(artifact.input().context().kind())||(symbol.flags()&java.lang.classfile.ClassFile.ACC_PRIVATE)==0;
    }
    private static boolean searchVisible(StoredArtifact artifact,Map<String,Object> symbol){
        return "local".equals(artifact.input().context().kind())||(flags(symbol)&java.lang.classfile.ClassFile.ACC_PRIVATE)==0;
    }
