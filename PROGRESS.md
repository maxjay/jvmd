...
### Modular source analysis and both resolver budgets

The first modular corpus run exposed a real file-manager bug: javac rejected the focused in-memory source when checking module membership. Focused source files now use the same location-aware representation as editor overlays; named dependency jars are resolved on the module path while their ordinary class bytes retain the index-backed LRU. A focused test compiles a real modular dependency, repeatedly attributes both the application descriptor and its classes, checks a new unsaved source, and verifies that an unexported dependency package remains inaccessible. The strict-AOT Spring graph performance/agreement test now runs against both native Maven bundles with the unchanged 1000 ms cold / 5 ms cached limits. CI validation pending.

### Evaluation deadlines and truthful budget errors

JDI method execution now has a five-second deadline for both interpreted and compiled evaluation. Because JDI cannot safely cancel an arbitrary target invocation, expiry terminates the owned application process, invalidates stopped frames, and reports that restart is required and external side effects can remain. Regression tests execute an infinite loop in both tiers and then launch the application again. Object-handle capacity errors now identify a releasable handle explicitly; expression-complexity errors no longer invent continuation cursors. LSP diagnostics normalize missing javac end positions so ranges cannot run backwards. CI validation pending.
...
### Independent response fragments

The nested Unicode fixture exposed multiplicative copying when several large fields were split independently while retaining the other fields on every fragment. Responses now separate independent large object fields first, retain existing segment offsets, and represent warnings-only fragments without inventing a payload. Candidate sizes use serialized UTF-8 bytes, including non-ASCII strings. The unchanged 4096-byte fixture and existing 64 KiB/MCP continuation tests gate exact reconstruction and limits. CI validation pending.

### Unbuilt named-module source overlays

Named dependency inputs now belong only to the module path, preventing an extra unnamed-module copy from influencing visibility diagnostics. Multiple unbuilt source modules are registered through the public module-source-path API, with module names parsed by javac. Editor buffers participate in each module location, including unsaved descriptor exports. The compiler only parses and analyzes; its required temporary output location is removed on close. A public-file-manager probe on the available JDK 17 confirms clean cross-module attribution, unsaved dependency type errors, descriptor export changes, and no generated classes. The authoritative JDK 25 tests additionally cover repeated pooled queries and non-exported packages; CI validation pending. Module registration was checked against the [JDK 25 StandardJavaFileManager API](https://docs.oracle.com/en/java/javase/25/docs/api/java.compiler/javax/tools/StandardJavaFileManager.html).

### Release-target module-location ownership

The unbuilt-module regression reproduced locally only when `--release` was enabled. OpenJDK's release file manager accepts any internal `LocationHandler` instance, even one created by a different file manager, so javac bypassed the editor-aware source lookup for module child locations. The analyzer now returns its own public `JavaFileManager.Location` wrappers and unwraps them only when delegating to its underlying manager. The local probe now reports the expected unsaved String-to-int error and observes changed module exports with `--release`; no additional compiler exports are needed. This was confirmed against JDK 25 [DelegatingJavaFileManager](https://github.com/openjdk/jdk25u/blob/master/src/jdk.compiler/share/classes/com/sun/tools/javac/main/DelegatingJavaFileManager.java) and [Locations](https://github.com/openjdk/jdk25u/blob/master/src/jdk.compiler/share/classes/com/sun/tools/javac/file/Locations.java). CI validation pending.

The independent response-fragment tests pass in run 34848203379, including the unchanged nested 4096-byte fixture and MCP replay. Native Maven 4 cold resolution repeated at 1013.491168 ms / 1000, with 331.77 ms bootstrap and 513.32 ms graph construction; cached p95 remains 2.826322 ms / 5. The cold margin remains an active implementation item.
...
### Maven 3.8.3, integration contract, and prebuilt distributions — 2026-09-15

Commit 92a4ffa adds real Maven 3.8.3 source-build and project-verification coverage, wrapper precedence, the developer integration guide, and the exported 14-tool catalog. Both Checkpoints jobs passed in run 34973810142, including live/verified agreement on PetClinic and jvmd. The embedded Maven 3 resolver remains 3.9.16 / Resolver 1.9.27.

The user then requested prebuilt downloads across operating systems. Commits b9958f0 and d61d157 add native Linux/macOS x64/arm64 archives, matching Java and Node runtimes, trained caches, checksum installers, Windows-to-WSL installation, and a separate Distributions workflow. Run 34977533774 passed all four native archive jobs, including forced AOT acceptance after relocation, unsaved LSP edits, URI identity, MCP discovery, and shared-daemon teardown. The first macOS run exposed /tmp versus /private/tmp alias handling; the fix canonicalizes owned source paths and preserves client URI spelling, with Java and Node regression coverage. The existing Checkpoints workflow is unchanged.

After CI measured Maven 4 cold resolution at 1085.163 ms against the 1000 ms assertion, the user explicitly requested changing the limit. The Maven 4 cold CI allowance is now 1500 ms. Maven 3 remains 1000 ms, cached p95 remains 5 ms for both majors, and every other benchmark and production deadline is unchanged. Benchmark measurements now include the applicable limits. The current follow-up also checks the Windows PowerShell installer with CRLF source files and unavailable-WSL handling. Final checks are recorded in PR #2.
