...
## 030 — Large-workspace A/B: baseline also fails, candidate adds retained pressure

- Exact isolated `IdentifierSweepTest` was run on pinned Temurin 25.0.4.1+1, Ubuntu 24.04 and `-Xmx1024m` against original baseline `ae23fd1f` and the semantic-state candidate. Both revisions complete the Petclinic corpus with exact identifier matches, then fail in the JVMD workspace with Rocks `Insert failed due to LRU cache being full` followed later by Java heap exhaustion.
- This disproves the earlier candidate-only hypothesis: the catastrophic large-workspace failure already exists in the original baseline and therefore is not, by itself, a reason to discard the semantic-Merkle architecture.
- The first JVMD `symbol.references` query is already pathological: ~19.1 s baseline and ~18.0 s candidate. Subsequent unchanged reference queries are ~4–6 s. The first Rocks LRU failure occurs after the seventh JVMD reference query in both revisions.
- Full sweep process RSS peaks at ~1,897,420 KiB baseline and ~1,918,344 KiB candidate. Baseline exits after ~6:54 wall time; candidate after ~7:40 and completes nine more JVMD reference requests before OOM.
- GC evidence shows a candidate retention cost that still needs attribution: baseline ends with ~914 MiB live after its last full compaction, candidate with ~1,020 MiB at a later query count; baseline spends ~26.3 s in parsed GC pauses versus ~59.9 s candidate. These end-of-run values are not a same-progress retained-heap comparison, so they establish pressure, not a precise +106 MiB branch delta.
- Rocks cache saturation precedes full-GC compaction in both revisions. The likely architecture loop is now narrower: large workspace semantic state is not being reused and/or local index preparation repeatedly walks/decodes workspace state, causing native-cache pressure first and heap pressure second.
- Code trace: `relationships` calls `prepareIndex` whenever the workspace snapshot object is not reused; `prepareIndex` refreshes every local module. `LocalArtifacts.refresh` walks and fingerprints all local source/output trees before it can prove unchanged. `RocksWorkspaceState.update` materialises persisted file/directory state during module-state configuration. If snapshot admission fails, a warm query can therefore cascade into whole-workspace semantic work plus whole-workspace index validation.
- [ ] Focused ten-query probe now samples existing `session.status` counters after each reference request on both revisions. Required proof: whether `workspace_bindings.builds` increments, `cached_files`/fragments remain empty, and Rocks native/workspace-state counters advance on unchanged queries.
