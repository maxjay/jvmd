...
class RocksSemanticInvalidationTest {
    ...
    @Test void bodyOnlyChangeStaysLocalButApiChangePropagatesTransitively()throws Exception{
        ...
        try(var state=new RocksSemanticInvalidation(temp.resolve("semantic"))){
            var initial=Map.of(
                    a,file(a,"content-a1","api-a",Set.of(),Set.of("pkg.A"),Set.of()),
                    b,file(b,"content-b1","api-b",Set.of(a),Set.of("pkg.B"),Set.of()),
                    c,file(c,"content-c1","api-c",Set.of(b),Set.of("pkg.C"),Set.of()));
            state.update("module","ctx",initial.values());

            var body=new LinkedHashMap<>(initial);body.put(a,file(a,"content-a2","api-a",Set.of(),Set.of("pkg.A"),Set.of()));
            var bodyResult=state.update("module","ctx",body.values());
            assertThat(bodyResult.bodyOnly()).containsExactly(a);
            assertThat(bodyResult.reanalyze()).containsExactly(a);

            var api=new LinkedHashMap<>(body);api.put(a,file(a,"content-a3","api-a2",Set.of(),Set.of("pkg.A"),Set.of()));
            var apiResult=state.update("module","ctx",api.values());
    ...
    @Test void cyclesTerminateAndUnresolvedTargetsInvalidateConservatively()throws Exception{
        ...
        try(var state=new RocksSemanticInvalidation(temp.resolve("cycle"))){
            var initial=Map.of(
                    a,file(a,"a1","api-a",Set.of(b),Set.of("pkg.A"),Set.of()),
                    b,file(b,"b1","api-b",Set.of(a),Set.of("pkg.B"),Set.of()),
                    u,file(u,"u1","api-u",Set.of(),Set.of("pkg.U"),Set.of("pkg.A")));
            state.update("module","ctx",initial.values());
            var changed=new LinkedHashMap<>(initial);changed.put(a,file(a,"a2","api-a2",Set.of(b),Set.of("pkg.A"),Set.of()));
            var result=state.update("module","ctx",changed.values());
    ...
    @Test void contextChangeInvalidatesAllAndDeletionInvalidatesDependants()throws Exception{
        ...
        try(var state=new RocksSemanticInvalidation(temp.resolve("context"))){
            var initial=Map.of(
                    a,file(a,"a1","api-a",Set.of(),Set.of("pkg.A"),Set.of()),
                    b,file(b,"b1","api-b",Set.of(a),Set.of("pkg.B"),Set.of()));
            state.update("module","ctx-1",initial.values());
            var context=state.update("module","ctx-2",initial.values());
            assertThat(context.contextChanged()).isTrue();
            assertThat(context.reanalyze()).containsExactlyInAnyOrder(a,b);

            var onlyB=List.of(file(b,"b1","api-b",Set.of(a),Set.of("pkg.B"),Set.of()));
    ...
    @Test void incrementalObservationPreservesOtherFilesAndClassifiesBodyVsApi()throws Exception{
        ...
        try(var state=new RocksSemanticInvalidation(temp.resolve("observe"))){
            ...
            var body=state.observeFile("module","ctx",file(a,"a2","api-a",Set.of(),Set.of("pkg.A"),Set.of()));
            assertThat(body.bodyOnly()).containsExactly(a);
            assertThat(body.reanalyze()).containsExactly(a);

            var api=state.observeFile("module","ctx",file(a,"a3","api-a2",Set.of(),Set.of("pkg.A"),Set.of()));
