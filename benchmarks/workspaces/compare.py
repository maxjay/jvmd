...
def compare(summary, candidate, baseline):
    fixtures = {}
...
def markdown(result):
    lines = ['# JVMD / JDTLS LSP benchmark', '',
             '| Fixture | Metric | JVMD | JDTLS | JVMD / JDTLS |',
             '|---|---|---:|---:|---:|']
    for fixture, metrics in result['fixtures'].items():
        for name, values in metrics.items():
            ...
            lines.append(f"| {fixture} | {name} | {values['jvmd']:.3f} | {values['jdtls']:.3f} | {ratio_text} |")
...
if __name__ == '__main__':
    ...
    args = parser.parse_args(); result = compare(json.loads(args.summary.read_text()), args.candidate, args.baseline)
