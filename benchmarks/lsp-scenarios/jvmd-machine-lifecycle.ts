...
async function main(){
  ...
  const residentDaemon={
    ...
    localIndexPublisherAfterSessions:settledIndex.sourcePublisher,
...
function markdown(report:any){
  ...
  const lines=[
    ...
    "| Artifact added → reconcile ready | - | - | "+fmt(incremental.artifactAddedToReadyMs)+" | - |",
    "",
    "A separate source-JAR update count is unavailable from current daemon status; aggregate reuse/hash counters and docs/source phase time are retained instead. Parse/storage timings are accumulated worker phase time and may exceed wall-clock scan time; do not sum phase rows into elapsed time.",
    "",
    ...
    "| Machine-index hashes during session open | "+sessions[0].workspaceIndex.machineIndexWorkDuringSessionOpen.hashed+" | "+sessions[1].workspaceIndex.machineIndexWorkDuringSessionOpen.hashed+" |",
    "| Machine-index reuses during session open | "+sessions[0].workspaceIndex.machineIndexWorkDuringSessionOpen.reused+" | "+sessions[1].workspaceIndex.machineIndexWorkDuringSessionOpen.reused+" |",
    "| Machine-index new publications during session open | "+sessions[0].workspaceIndex.machineIndexWorkDuringSessionOpen.indexedPublications+" | "+sessions[1].workspaceIndex.machineIndexWorkDuringSessionOpen.indexedPublications+" |",
    "| Machine-index parse worker-time during session open ms | "+fmt(sessions[0].workspaceIndex.machineIndexWorkDuringSessionOpen.parseMs)+" | "+fmt(sessions[1].workspaceIndex.machineIndexWorkDuringSessionOpen.parseMs)+" |",
    "| Machine-index storage worker-time during session open ms | "+fmt(sessions[0].workspaceIndex.machineIndexWorkDuringSessionOpen.storageMs)+" | "+fmt(sessions[1].workspaceIndex.machineIndexWorkDuringSessionOpen.storageMs)+" |",
