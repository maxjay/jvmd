...
def verify(root):
    counts = {'workers': 0, 'responses': 0, 'ranges': 0, 'dependency_identity_sets': 0}; identity_sets = {}
        ...
        for path in sorted(report_path.parent.glob('process*/messages.jsonl')):
            ...
            for line in path.read_text().splitlines():
                ...
                if record['direction'] == 'send' and 'method' in message and 'id' in message: pending[message['id']] = message
                    ...
                    probe = source_path(request['params']['textDocument']['uri']); expected = set(probe.parent.glob('*.java'))
                    if method == 'textDocument/references': locations = value
                    elif method == 'textDocument/definition':
                        locations = [value] if isinstance(value, dict) else value
                        assert len(locations) == 1 and source_path(locations[0]['uri']).name.endswith('_0.java'), value
                    else:
                    ...
                    actual = [source_path(location['uri']) for location in locations]
