...
- The [checkpoint job passes](https://github.com/maxjay/jvmd/actions/runs/35466342195), including the previously failing compiler test. [All four native packaging/AOT/relocation jobs and Windows installer pass](https://github.com/maxjay/jvmd/actions/runs/35466342198). The separate corpus job is pending at the final validation check.
- An earlier [full checkpoint run](https://github.com/maxjay/jvmd/actions/runs/35465315484) exposed a directory-walk race in `IncrementalDiagnosticsStoreTest`: an unrelated diagnostics snapshot temporary file disappeared during source discovery. Follow-up commit `ad8181acd8e86264c430609b083612f322c2672a` replaces `Files.find` in `Application.sourceFiles` with the existing `FileInventory.matching` visitor, which tolerates concurrent removal while preserving other I/O errors. That existing failing test remains unchanged and now passes. This small follow-up was not included in the recorded timings because the execution environment is offline. No tests, thresholds or gates were weakened.
