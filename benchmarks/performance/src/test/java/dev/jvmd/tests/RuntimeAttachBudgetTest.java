...
class RuntimeAttachBudgetTest {
    ...
    @Test void productionAttachMeetsTheBudget()throws Exception{
        ...
        try(var daemon=new AotDaemon(root,Map.of("jdk_home",System.getProperty("java.home")))){
            ...
            var run=daemon.request("run.start",Map.of("session",session,"target","Probe","debug",true)).path("result");double attach=run.path("attach_ms").asDouble();assertThat(run.path("port").asInt()).isPositive();assertThat(attach).isGreaterThan(0);
            var metrics=Map.of("attach_ms",attach,"observation_target_ms",500);System.out.println("phase-10-perf "+Json.MAPPER.writeValueAsString(metrics));Json.MAPPER.writeValue(TestSupport.repo().resolve("jvmd-tests/target/phase-10-perf.json").toFile(),metrics);
