...
Phase 9 adds no dependency. The LSP facade depends only on jvmd-core; the TypeScript bridge uses Node's built-in modules. Protocol shapes were checked against the [official LSP 3.17 sources](https://github.com/microsoft/language-server-protocol/tree/gh-pages/_specifications/lsp/3.17). Completion attribution uses public JDK 25 Trees/Scope/Types APIs to detach document and type semantics into JVMD's resident semantic state; ordinary completion queries that detached state. Signature help still uses public Trees.getScope/isAccessible, Scope, Elements.getAllMembers and Types.asMemberOf. These APIs were verified in the [JDK 25 Trees documentation](https://docs.oracle.com/en/java/javase/25/docs/api/jdk.compiler/com/sun/source/util/Trees.html) and [Types documentation](https://docs.oracle.com/en/java/javase/25/docs/api/java.compiler/javax/lang/model/util/Types.html). No extra javac exports are introduced.
