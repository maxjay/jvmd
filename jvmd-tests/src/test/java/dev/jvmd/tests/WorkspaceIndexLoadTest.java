...
class WorkspaceIndexLoadTest {
 ...
 @Test void warnsAboutDuplicatesAndFiltersWorkspace()throws Exception{
  ...
  try(var index=new IndexService(temp.resolve("index.db"),temp)){index.indexJar(a,"fixture:a:1","jar");index.indexJar(b,"fixture:b:1","jar");
   ...
   assertThat(index.store().counts().get("edges")).isGreaterThan(0L);
  }
 }
}
