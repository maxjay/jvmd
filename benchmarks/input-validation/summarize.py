...
for scenario in data['baseline'][0]:
    for metric in ['validation_ms','navigation_ms','thread_allocated_bytes','file_loads','work.source_enumerations','work.classpath_enumerations','work.files_hashed','work.bytes_hashed','work.metadata_checks','work.identity_map_rebuilds']:
        samples={}
...
for scenario in diagnostics['baseline'][0]:
    for metric in ['diagnostics_ms','queries','diagnostic_files_analysed','diagnostic_files_reused','thread_allocated_bytes']:
        for percentile in ([50,95] if metric.endswith('_ms') else [None]):
            values={}
            for label,runs in diagnostics.items():
                samples=[run[scenario][metric] for run in runs]
