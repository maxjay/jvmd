...
public final class Documentation {
    ...
    private static final String REACH="WITH RECURSIVE reach(id,d) AS (SELECT ?,0 UNION SELECT e.dst,r.d+1 FROM edges e JOIN reach r ON e.src=r.id WHERE r.d<? AND e.kind IN ("+KINDS+")) ";
    ...
    private List<Long> reachable(long root,int depth,int limit,int offset)throws Exception{
        return index.database().read(c->{var result=new ArrayList<Long>();try(var q=c.prepareStatement(REACH+"SELECT id,min(d) distance FROM reach GROUP BY id HAVING min(d)>0 ORDER BY distance,id LIMIT ? OFFSET ?")){
            q.setLong(1,root);q.setInt(2,depth);q.setInt(3,limit);q.setInt(4,offset);try(var r=q.executeQuery()){while(r.next())result.add(r.getLong(1));}
        }return result;});
    }
    private void completeJdk(long root,int depth)throws Exception{
        for(int pass=0;pass<depth;pass++){
            var needed=index.database().read(c->{var result=new LinkedHashSet<String>();try(var q=c.prepareStatement(REACH+"SELECT DISTINCT t.target FROM reach r JOIN edge_targets t ON t.src=r.id WHERE r.d<? AND t.kind IN ("+KINDS+") AND NOT EXISTS(SELECT 1 FROM symbols s WHERE s.binary_key=t.target) LIMIT 1000")){
                q.setLong(1,root);q.setInt(2,depth);q.setInt(3,depth);try(var r=q.executeQuery()){while(r.next())result.add(r.getString(1));}
            }return result;});
    ...
    private String inherited(long id,String workspace,Set<Long> visited)throws Exception{
        ...
        var parents=index.database().read(c->{var result=new ArrayList<Long>();try(var q=c.prepareStatement("SELECT dst FROM edges WHERE src=? AND kind='overrides' ORDER BY dst LIMIT 100")){q.setLong(1,id);try(var r=q.executeQuery()){while(r.next())result.add(r.getLong(1));}}return result;});
    ...
    public Envelope describe(Map<String,Object> symbol,String workspace,String detail,int depth,int limit,int offset)throws Exception{
        ...
        if(depth>0&&symbol.get("id") instanceof Number number){
            long id=number.longValue();completeJdk(id,depth);
            // Filter by the workspace before applying the page size; inaccessible versions cannot consume a page.
            int scan=offset;while(closure.size()<=limit){var ids=reachable(id,depth,limit+1,scan);if(ids.isEmpty())break;for(long next:ids){scan++;var member=index.byId(next,workspace);if(member==null)continue;closure.add(documented(member,detail,workspace));if(closure.size()>limit){scan--;more=true;break;}}if(more||ids.size()<limit+1)break;}
