...
public final class NamespaceResolutionProofs {
    ...
    public static Plan plan(String source,String simpleName,String resolvedBinary){
        ...
        String pkg="";
        var packageMatcher=PACKAGE.matcher(source);if(packageMatcher.find())pkg=packageMatcher.group(1);

        var explicit=new TreeSet<String>();var wildcard=new TreeSet<String>();
    ...
    private static String normalize(String binaryName){return binaryName.replace('$','.');}
        ...
        onDemand.remove(pkg);
        for(String namespace:onDemand)domains.add(namespace+"."+simpleName);
        return new Plan(simpleName,resolvedBinary,List.copyOf(new LinkedHashSet<>(domains)));
    }
