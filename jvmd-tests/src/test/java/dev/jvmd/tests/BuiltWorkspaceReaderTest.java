...
class BuiltWorkspaceReaderTest {
    ...
    @Test void sourceContentChangesWithPreservedMtimeSwitchToSource()throws Exception{
        ...
        var reader=new WorkspaceOverlay(List.of(module),true);var requested=new DefaultArtifact("fixture:library:jar:1");
    ...
    @Test void cyclesAreStatusErrorsAndStrictVersionsDoNotSubstitute(){
        ...
        var reader=new WorkspaceOverlay(List.of(a,b),false);assertThat(reader.status().get("errors").toString()).contains("overlay_cycle");assertThat(reader.findArtifact(new DefaultArtifact("fixture:a:jar:2"))).isNull();
    }
}
