...
### Checkpoint 18 — confirmed warm-query and restart gains
- Completed 24 controlled JVMD workers comparing previous Rocks `deefc256` with optimized `3871bc83`, and 12 pinned JDTLS workers on identical fixture bytes. Every JVMD worker has zero faults, all type identity hashes agree, and all 12 reopen workers rebuild zero artifacts.
- Parent-process launch through type readiness (previous/optimized Rocks): one JAR fresh 12.004/9.911 s, restart 2.452/0.561 s; 128 JARs fresh 5.318/3.488 s, restart 2.778/0.775 s. The measured query-planning waste was materially narrowing the previous JDTLS comparison.
- Broad warm type queries after the fresh build fall from 1,704.84 to 9.19 ms for 950 types and 1,770.91 to 18.29 ms for 1,024 types. After restart they fall from 1,600.55/1,730.83 ms to 9.29/24.22 ms. Mixed-kind searches and exact-query p95 do not receive a universal improvement claim; the 128-result fresh exact-query p95 is worse in this fixed short warm-up batch.
- JDTLS fresh/restart readiness is 7.731/3.716 s for one JAR and 7.809/3.587 s for 128 JARs. Broad warm type search after a fresh build is 219.85/131.77 ms. JVM launch is now included for both implementations, but JVMD still uses direct index APIs while JDTLS includes LSP and project/JDK startup. Both systems do upfront indexing; the evidence does not support treating JDT as unindexed.
- Seed scan ranges overlap: one JAR 9.670/9.429 s median and 128 JARs 2.930/3.097 s (previous/optimized). These changes do not establish a material seed-speed improvement. A diagnostic allocation profile estimates merge decoder churn at 1.619/0.306 GB, about 81% less. Restart peak RSS drops from 375/377 MB to 264/258 MB across the fixtures.
- The optimized code passed the complete checkpoint job in `35445446148` and all four native package/AOT/relocation jobs plus Windows installer in `35445446248`. Its serialized corpus job remains pending. The PR stays ready for review; the final report-only head triggers separate CI.
- Raw results, pinned JDT source audit, exact revision mappings, allocation evidence and reproduction commands are linked from `docs/performance/2026-09-19-query-optimization.md`.

### Checkpoint 19 — seed allocation and merge follow-up
- Production `0c8b5a50` (local `4e642f5`, tree `fb4fff25`) binds each sorter to one artifact namespace, constructs relative keys once, reuses symbol IDs/gram tables/checksum buffers, avoids singleton posting-key copies, and drains ordered merge-run ranges without per-row queue reinsertion. Format, validation and memory budgets remain unchanged.
- Five alternating before/after repetitions, 40 JVMs: large-JAR service open plus scan falls from 10.253 to 9.366 s (8.7%), with all five pairs faster and non-overlapping observed ranges. The 128-JAR fixture is effectively unchanged at 3.628/3.581 s with overlapping ranges and mixed pairs. Query and restart results are mixed; no universal speedup claim.
- Record preparation, initial sort/spill, and merge/SST medians fall from 2.717/1.667/3.502 s to 2.468/1.478/3.108 s. Sort record counts and ~33 MB spill volume are unchanged. One diagnostic JFR per version estimates 6.194/4.904 GB of transient allocation (~21% less), with 48/38 GCs. Single-JAR peak RSS rises from 855.5 to 873.3 MB; allocation churn is not retained memory.
- All 75 selected tests pass (47 storage, 28 integration), including independent full-key/checksum equivalence and cross-run duplicate rejection. All 40 timed workers have zero faults and matching type identities; 20 normal restarts and four bidirectional cross-version fixture opens rebuild zero artifacts.
- Refreshed 12-process JDTLS comparison: JVMD/JDTLS process-to-types is one-JAR fresh 9.495/7.925 s, restart 0.601/3.835 s; 128-JAR fresh 3.822/7.988 s, restart 0.849/3.628 s. JDTLS still wins single-JAR fresh readiness; both systems preindex. JVMD direct APIs and JDTLS LSP/project/JDK duties remain different scopes.
- Production code passes all four native distribution jobs and Windows installer in `35446737297`. Full checkpoints also pass in `35446737295`, with serialized corpus pending. The PR remains ready for review. Raw results, commands, class/revision hashes, allocation and compatibility evidence are linked from `docs/performance/2026-09-19-seed-optimization.md`.

### Checkpoint 20 — post-merge gram, merge and bounded-page performance
- Started from merged main `dce59413`, tree `a550c895`, on `perf/compact-grams-merge`. Fresh profiles identified gram strings/set nodes, merge ID arrays, final-key buffers and full symbol decoding for out-of-page candidates. Production `c0739d14` matches local `edb807e`, tree `f13ee5ea`.
- Added primitive UTF-16 gram sets, immutable stored map keys with a reusable lookup probe, fully validated merge cursors, bounded posting encoders and same-length SST key reuse. Plain signature generations reject unusable local IDs before reading facts; natural-ID enumeration seeks to the cursor and stops after filling the page. Enriched generations retain symbol-based ranking. Persisted format and configured budgets are unchanged.
- Five alternating repetitions: first 20-row mixed-kind substring pages fall from 1,290.259 to 7.696 ms (~168x); second/middle pages are 1,283.518/10.448 and 1,271.692/3.810 ms. Blank enumeration is about 0.64–0.69 ms versus 1.45–1.46 s. All full-page hashes match; optimized calls decode exactly 20 facts per request.
- Generated one-JAR seed falls 9.048 to 8.530 s (5.7%, all five pairs faster but overlapping ranges). 128-JAR seed is 3.032/2.915 s with mixed pairs. Five real dependencies (18,997 symbols) are effectively unchanged at 1.448/1.425 s. Full-result queries are mixed, including a fresh 128-result exact p95 increase from 14.721 to 18.812 ms. These tradeoffs remain in the report.
- Matched diagnostic profiles estimate 5.042/3.541 GB allocated (~29.8% less); peak RSS remains mixed. All 80 selected tests pass (52 storage, 28 integration), including randomized UTF-16 equivalence, LRU eviction, corruption, page limits, custom ranks and enriched-ID pagination. All 40 generated, 20 real-dependency and 10 page workers pass; 30 generated/real restarts plus four cross-version opens rebuild zero artifacts. Both complete Maven update sequences pass.
- Measured production passes full checkpoints in `35455511165` and all native packaging/installer jobs in `35455511150`; corpus agreement is still running. Raw evidence and reproduction are in `docs/performance/2026-09-19-compact-grams-and-pages.md`. PR #7 is the review vehicle. Remaining corpus/environment and concurrency/body/diagnostic acceptance stays separate.
