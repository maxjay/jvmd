...
public final class Analyzer implements DiagnosticEngine, AutoCloseable {
    ...
    public Envelope overview(Path path,String text,int depth,int limit,int offset)throws Exception{
        ...
        var result=compiler.query(path,text,1,(task,units,tier)->Bindings.outline(task,units,new SymbolIdentity(task,context.gav(),context.release(),this::coordinates,context.navigationSources()),path,sourceText(path,text),depth));
