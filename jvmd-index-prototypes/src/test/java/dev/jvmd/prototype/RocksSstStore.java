...
final class RocksSstStore implements PrototypeStore {
    ...
    @Override public void publish(List<Entry> entries)throws Exception{
        ...
        try(var env=new EnvOptions();var writer=new SstFileWriter(env,options)){
            writer.open(sst.toString());for(var entry:sorted)writer.put(entry.key(),entry.value());writer.finish();
        }
        try(var ingest=new IngestExternalFileOptions().setMoveFiles(false).setSnapshotConsistency(true).setWriteGlobalSeqno(false)){
    ...
    @Override public List<Entry> prefix(byte[] prefix,int limit){
        var result=new ArrayList<Entry>();try(var iterator=db.newIterator()){
            for(iterator.seek(prefix);iterator.isValid()&&result.size()<limit;iterator.next()){
                byte[] key=iterator.key();if(!ImmutableSegmentStore.startsWith(key,prefix))break;result.add(new Entry(key,iterator.value()));
            }
        }return List.copyOf(result);
    }
    @Override public long storageBytes()throws Exception{try(var files=Files.walk(root)){return files.filter(Files::isRegularFile).mapToLong(p->{try{return Files.size(p);}catch(Exception e){return 0;}}).sum();}}
