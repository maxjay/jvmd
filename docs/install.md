...
Benchmark observations are not production cutoffs; see the [benchmark guide](../benchmarks/workspaces/README.md). Prebuilt users do not run the benchmark suite. Operational deadlines prevent hangs: adapter startup is 10 seconds, adapter requests six minutes, verified builds five minutes, and debugger method evaluation five seconds. Evaluation timeout can terminate the debugged application; see the [integration contract](integration.md#runtime-and-debugging).
