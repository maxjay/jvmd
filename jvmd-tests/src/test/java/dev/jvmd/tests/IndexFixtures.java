...
final class IndexFixtures {
    static Path jar(Path directory,String name,String source,boolean parameters)throws Exception{
        Files.createDirectories(directory);Path input=directory.resolve("Sample.java"),classes=Files.createDirectories(directory.resolve(name+"-classes"));Files.writeString(input,source);
