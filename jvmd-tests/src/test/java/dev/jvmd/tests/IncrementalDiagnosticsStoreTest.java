...
class IncrementalDiagnosticsStoreTest {
    ...
    @Test void unchangedWorkspaceAndPaginationPerformZeroAdditionalJavacQueries()throws Exception{
        for(int i=0;i<40;i++)Files.writeString(temp.resolve("Source"+i+".java"),"class Source"+i+" { int value() { return \"bad\"; } }\n");
        try(var daemon=new AotDaemon(temp)){
            ...
            daemon.request("diag.get",Map.of("session",session,"limit",10,"cursor",page1.path("cursor").asText()));
