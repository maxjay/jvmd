...
public final class DiagnosticSnapshots implements AutoCloseable {
    ...
    public DiagnosticStore.State restore(DiagnosticStore.Key key){
        try{
            ...
            hits++;return new DiagnosticStore.State(envelope,data.path("api").asText(),Set.copyOf(dependencies),data.hasNonNull("contribution")?Json.MAPPER.treeToValue(data.get("contribution"),dev.jvmd.index.FileSemanticContribution.class):null);
    ...
    public void save(DiagnosticStore.Key key,DiagnosticStore.State state,CompilerInputs.Snapshot inputs){
        try{
            ...
            data.put("contribution",state.contribution());data.put("dependencies",dependencies);data.put("tier",state.diagnostics().tier());data.put("warnings",state.diagnostics().warnings());
